...
namespace :deploy do
  ...
  task :check_revision do
    ...
    unless `git rev-parse HEAD` == `git rev-parse %{git_remote_name}/master`
      ThorUtil.failure(
  ...
  task :undo do
    on roles(:app) do
      ...
      # From template
      puma_name = '%{puma_name}'.freeze
      remote_app_base_dir = '%{remote_app_base_dir}'.freeze
      remote_git_base_base_dir = '%{remote_git_base_dir}'.freeze
